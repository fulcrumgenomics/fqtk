...
use fqtk_lib::read_set::{ReadSet, ReadSetIterator, SkipReason};
        ...
        for output_type in output_types {
            ...
            for idx in 1..=segment_count {
                output_type_writers.push(BufWriter::new(File::create(
                    output_dir.join(format!("{prefix}.{file_type_code}{idx}.fq.gz")),
                )?));
            }
    ...
    fn execute(&self) -> Result<()> {
        ...
        loop {
            ...
            assert_eq!(
                next_read_sets.len(),
                fq_iterators.len(),
                "FASTQ sources out of sync at records: {next_read_sets:?}"
            );
        ...
        if skip_reasons.is_empty() {
            ...
            for (reason, count) in skip_reasons.iter().sorted_by_key(|(_, c)| *c) {
                info!("{count} records were skipped due to {reason}");
            }
        }
...
mod tests {
    ...
    fn metadata_lines_from_barcodes(barcodes: &[&str]) -> Vec<String> {
        ...
        for (i, &barcode) in barcodes.iter().enumerate() {
            result.push(format!("Sample{i:04}\t{barcode}"));
        }
        result
    }
